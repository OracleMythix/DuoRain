...
def fetch_streak_start():
 r=requests.get(PROFILE_URL,headers=HEADERS);r.raise_for_status()
 info=r.json()
...
"""+Fore.LIGHTBLACK_EX+"           ~ Storm 🌪️\n")
 ...
 if mode.endswith('s'):mode=mode[:-1]

 if mode=="streak":prompt="How many days you want to farm? "
 elif mode in("xp","gem"):prompt="How many loops you want to farm? "
 else:print(Fore.RED+"Unknown option. Use 'xp', 'gems', or 'streak'.");exit(1)
